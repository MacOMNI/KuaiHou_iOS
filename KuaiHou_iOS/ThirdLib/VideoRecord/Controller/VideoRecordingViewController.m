...
- (void)rightButtonClick:(UIButton *)btn{
    ...
    if (!btn.selected) {
        ...
        [self.manager getVideoAndThumbnailPathWithBlock:^(NSString *videoPath, NSString *thumbnailPath) {
            if (self.finshRecord) {
                self.finshRecord(videoPath, thumbnailPath, [self.manager getRecordTime]);
            }
...
- (void)recordTimerTooShort:(VideoRecordManager *)manager{
    dispatch_async(dispatch_get_main_queue(), ^{
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"录制时间过低" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alertView show];
        [self deleteVideoRecord];
    });
}
