...
- (void)onActionWithDidSelect
{
    ...
    //这里可以做处理。假如是要求索引齐全 但是 section不齐全 则多设置一个真实section数组进行位移
    //判断数组进行比较 当前的currentSection的字母 循环对真实数组比较 如果有 就移动到真实数组的下标
    NSLog(@"%d -%d",_currentSection,_tableView.numberOfSections);
   // if (_currentSection>_headIndexArray.count) {//大于真实数组的个数 count
 //       return;
 //   }
