...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self == [super initWithFrame:frame]) {
        ...
        self.cycleScrollView.urlImages = imagesURLStrings;
        
        kWEAKSELF;
        [self.cycleScrollView setDidScrollIndexBlock:^(NSInteger index) {
            weakSelf.numberLab.hidden = NO;
            weakSelf.numberLab.text = [NSString stringWithFormat:@"%ld/%ld", index + 1, weakSelf.bannerView.imageURLStringsGroup.count];
        }];
...
-(SPCycleScrollView *)cycleScrollView{
    if (!_cycleScrollView) {
        _cycleScrollView = [[SPCycleScrollView alloc] init];
        _cycleScrollView.frame = self.bounds;
        _cycleScrollView.pageControl.hidden = YES;
        _cycleScrollView.imageMode = UIViewContentModeScaleAspectFill;
