...
@implementation MyDynamicsPhotosView
...
- (void)layoutSubviews {
    ...
    for (int i = 0; i<photosCount; i++) {
        ...
        int col = i % maxCol;
        photoView.x = 40 + col * ((kScreenSizeWidth-85)/3 + PhotoMargin);
        
        int row = i / maxCol;
        photoView.y = 5 + row * ((kScreenSizeWidth-85)/3 + PhotoMargin);
        photoView.width = (kScreenSizeWidth-85)/3;
        photoView.height = (kScreenSizeWidth-85)/3;
        //
...
+ (CGSize)sizeWithCount:(NSUInteger)count
{
    ...
    CGFloat photosW = cols *  (kScreenSizeWidth-85)/3 + (cols - 1) * PhotoMargin;
    
    // 行数
    NSUInteger rows = (count + maxCols - 1) / maxCols;
    CGFloat photosH = rows *  (kScreenSizeWidth-85)/3 + (rows - 1) * PhotoMargin;
