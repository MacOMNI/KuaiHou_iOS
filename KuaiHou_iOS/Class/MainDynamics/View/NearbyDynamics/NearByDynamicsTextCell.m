...
@interface NearByDynamicsTextCell()
...
@implementation NearByDynamicsTextCell
...
-(void)fixText:(NSAttributedString *)text isOpen:(BOOL)isOpen{
    ...
    container.size = CGSizeMake(kScreenSizeWidth-30, CGFLOAT_MAX);
#pragma mark 这里控制是否显示全文
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self->_contentLabel.frame = CGRectMake(15, 0, layout.textBoundingSize.width, layout.textBoundingSize.height);
...
+(CGFloat)cellHeightWithContent:(NSString *)content WithFont:(CGFloat)font isOpen:(BOOL)isOpen
{
    ...
    NSMutableAttributedString *text = [[NSMutableAttributedString alloc] initWithString:content];
    [text yy_setFont:[UIFont systemFontOfSize:font] range:text.yy_rangeOfAll];
    text.yy_lineSpacing = 5;
    YYTextContainer *container = [YYTextContainer new];
    container.size = CGSizeMake(kScreenSizeWidth-30, CGFLOAT_MAX);
    //    container.maximumNumberOfRows = 5;
